...
# We want to know if the murder rate is less or equal than in Italy:
ind <- murder_rate <= 0.71
# To see which states these are, use index with logicals:
murders$state[ind]
# Count how many are TRUE by sum() which returns the sum of the entries of a vector and logical vectors get coerced to numeric with TRUE coded as 1 and FALSE as 0.
sum(ind)
# Check how many states have murder rate below average:
...
murder_rate[ind]
# Get the indexes of entries that are below 1:
which(murder_rate < 1)
...
library(dplyr)
# Add the murder rate to the data frame with mutate():
...
# Create a table that satisfies both the conditions: it is in the Northeast or West and the murder rate is less than 1. 
